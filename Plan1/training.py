            ...
            for _, batch in enumerate(loader_train):
                ...
                if args.mixed_precision:
                    with torch.cuda.amp.autocast():
                        log_probs, ss_logits = model(X, S, mask, chain_M, residue_idx, chain_encoding_all)
                        _, loss_av_smoothed = loss_smoothed(S, log_probs, mask_for_loss)

                        # print("[DEBUG] ss_logits has nan:", torch.isnan(ss_logits).any().item())
                        # print("[DEBUG] ss_logits max:", ss_logits.max().item(), "min:", ss_logits.min().item())
                        # print("[DEBUG] ss_logits.shape:", ss_logits.shape)
                        # print("[DEBUG] ss_true unique values:", torch.unique(ss_true))
